<?php
	...
	if(isset($_SESSION['admin_username']))
	{
		...
		$sql = "SELECT * FROM directors WHERE user_type = '1' AND manual_entry != 1 AND state='".$_GET['state']."' AND city='".$data['city_id']."'  ORDER BY id DESC";
		
		}
		else
		{
			$sql = "SELECT * FROM directors WHERE user_type = '1' and manual_entry != 1 ORDER BY id DESC";
		}
...
<div class="content">
<div class="header">
  <h1 class="page-title">Basic Members</h1>
</div>
<ul class="breadcrumb">
...
<div class="well">
  <table class="table">
    ...
    <tbody>
      ...
      <tr>
        <td><?php echo $i;?></td>
        <td style="width:80px;"><?php echo date('d-M-y',strtotime($data['date']));?> </td>
...
<?php
				...
				?>              </td>
        <td><!--<a href="<?php echo base_url;?>admin/members/edit_clinet.php?id=<?php echo $data['id'];?>"><i class="icon-pencil"></i></a>-->
          <a href="javascript:void(0);" member_id="<?php echo $data['id'];?>" class="delete" role="button"><i class="icon-remove"></i></a> </td>
      </tr>
      <?php
	 			$i++;
		}
