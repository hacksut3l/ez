...
<?php 
		...
		$data_of_location=mysql_query("select * from directors where id='".$_SESSION['client']."'");
		$result_of_location=mysql_fetch_array($data_of_location);
		$totallocation=$result_of_location['total_location'];
		$_SESSION["total_location"]=$totallocation-1;
		$locamount = $_SESSION["total_location"] * 20;
		
		$update_account=mysql_query("select * from director_member_info where director_id='".$_SESSION['client']."'");
	
		$payment_counter=mysql_num_rows($update_account);
		$update_result=mysql_fetch_array($update_account);
		
		if($payment_counter > 0 && $update_result['order_info'] =='Approved' && $result_of_location['user_type']==3)
		{
			
			$final_amount = $locamount + 0;
...
if ( isset($_POST['btnSubmit']) ) {
    ...
    if ($_POST['ddlMethod'] == 'ProcessPayment' || $_POST['ddlMethod'] == 'Authorise' || $_POST['ddlMethod'] == 'TokenPayment') {
        ...
        $request->Payment->TotalAmount = $final_amount1;
...
<div class="gridContainer clearfix"><br />
  ...
  <div id="LayoutDiv1">
    <div id="wrapper">
      <div id="container">
        <div class="container">
          <div class="login_wrapper" style="border:2px solid #c2c2c2; margin-bottom:65px; ">
            <?php
...
$payment_status=mysql_query("UPDATE `directors` SET `payment_status`='active' where id='".$_SESSION['client']."' or reference_id='".$_SESSION['client']."'");
...
?>
		...
		<span class="verify_subtitle"  style="font-family: Arial, Helvetica, sans-serif !important;">Please Try Again....!</span>
            <span class="verify_subtitle"  style="font-family: Arial, Helvetica, sans-serif !important;"><a href="premium-membership-registration-payment.php" style="font-size:24px !important;">Back</a></span>
            <?php		
	}
...
?>
            ...
            <form method="POST" id="free_next_frm">
              ...
              <span class="login_head director_heading"  style="font-size:32px;">Premium Membership Application <span class="step">Step 2 of 2</span></span>
              <div class="left_reg_form_next"> <span class="fields_wrapp marginTop"> <span class="reg_name ename">Below fee will immediately be charged to your credit card, and will be charged each and subsequent month that you continue your membership as a Premium Member. When eziFunerals provides you with a pre-qualified lead (which includes the individual’s full name, email address, phone number, and funeral details) an additional $29 per lead is what you agree to be charged per lead provided, tallied at the end of each calendar month for all leads provided.</span> </span> <span class="fields_wrapp marginTop tablr_bor">
				...
				?> 
                 ...
                 <?php } ?>
				  <tr>
                    <td class="reg_table_text">Total Locations</td>
                    <td class="reg_table_text"><?php echo $_SESSION["total_location"] ;?> * $20 = <?php echo '$'.$locamount;?></td>
                  </tr>
                  <tr>
                    <td class="reg_table_text"><strong>Total Due Today</strong></td>
                    <td class="reg_table_text"><strong>$<?php echo $final_amount;?>.00</strong></td>
                  </tr>
                </table>
